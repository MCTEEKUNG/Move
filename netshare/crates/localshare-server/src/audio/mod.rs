...
impl ServerAudio {
    ...
    /// Like `start()` but falls back to a no-op stub if audio devices are
    /// unavailable (e.g. headless server). Safe to call from the daemon.
    pub fn start_or_stub() -> Self {
        match Self::start() {
            Ok(audio) => audio,
            Err(e) => {
                tracing::warn!("Audio subsystem unavailable (running stub): {e}");
                Self { mic_target: Arc::new(Mutex::new(None)) }
            }
        }
    }
